...
var socialConfig = nconf.get("socialConfig");
...
describe('toolbar data', function() {
    ...
    var sessionScope;
    var coursesScope;
    var notificationsScope;

    before(function(){
        scope = nock('https://admin.pearsonopenclass.com')
            .get('/service/me.json')
            .replyWithFile(200, __dirname + '/../data/whitaker.me.isSocial.json', {'Content-type': 'application/json'});
        ...
        notificationsScope = nock(nconf.get('socialConfig:notifications_root_url'))
            .get('/notifications/all/user/berlin_4f9ff07100c40fc408d497de')
            .reply(200, { code: 200, status: 'success', data : { notifications: [] } }, {'Content-type': 'application/json'});
    });

    it('should contain social behavior and config', function(done){
        ...
        request.get(url, function (error, response, result) {
            ...
            should.strictEqual(socialConfig.settings_url, data.config.settings_url);;
            should.strictEqual(socialConfig.dashboard_url, data.config.dashboard_url);
            should.strictEqual(socialConfig.profile_url, data.config.profile_url);
            should.strictEqual(socialConfig.admin_url, data.config.admin_url);
            should.not.exist(data.config.share_url);
            data.institution.is_social.should.be.true;
            done();
        });
    })
});
...
describe('toolbar data', function() {
    ...
    before(function(){
        scope = nock('https://admin.pearsonopenclass.com')
            .get('/service/me.json')
            .replyWithFile(200, __dirname + '/../data/whitaker.me.notSocial.json', {'Content-type': 'application/json'});
        chamberScope = nock(nconf.get('CHAMBER_ROOT_URL'))
            .get('/me/google_oauth2')
            .reply(404, { code: 404, status: 'success', data: {} }, {'Content-type': 'application/json'});
        assertionScope = nock(nconf.get('WSOD_M_ROOT_URL'))
    ...
    it('should not contain social behavior', function(done){
        var url = rootUrl + '/toolbar';
        request.get(url, function (error, response, result) {
            result = JSON.parse(result);
            should.strictEqual(null, error);
            should.exist(result);
            should.strictEqual('success', result.status);
            result.data.institution.is_social.should.be.false;
            done();
        });
    })
});